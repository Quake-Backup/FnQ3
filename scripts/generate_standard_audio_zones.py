...
def default_tool_path() -> Path:
    return Path(os.environ.get("FNQ3_AUDIOZONESC", "fnq3-audiozonesc"))
        ...
        with zipfile.ZipFile(pak) as archive:
            arena_entries = sorted(
                (
                    ...
                    if entry == "scripts/arenas.txt"
                    or (entry.startswith("scripts/") and entry.endswith(".arena"))
                ),
                key=str.lower,
            )
...
def extract_standard_bsp_files(
    ...
    wanted = {name.lower(): name for name in map_names}
    found: set[str] = set()
    for pak in paks:
        with zipfile.ZipFile(pak) as archive:
            entries = {entry.lower(): entry for entry in archive.namelist()}
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate tracked FnQuake3 .azb sidecars for standard Quake III "
    ...
    parser.add_argument(
        ...
        help="Path to fnq3-audiozonesc. Defaults to FNQ3_AUDIOZONESC or PATH lookup.",
    )
    parser.add_argument(
        "--work-root",
        type=Path,
