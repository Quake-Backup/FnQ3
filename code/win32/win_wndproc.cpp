...
char *Sys_GetClipboardData( void ) {
	...
	fnq3::win::ScopedClipboard clipboard( nullptr );
	if ( clipboard ) {
		...
		if ( ( hClipboardData = GetClipboardData( CF_TEXT ) ) != 0 ) {
			fnq3::win::ScopedGlobalLock<char> cliptext( hClipboardData );
			if ( cliptext ) {
...
void Sys_SetClipboardData( const char *text )
{
	...
	fnq3::win::ScopedClipboard clipboard( g_wv.hWnd );
	if ( !clipboard )
		return;
	...
	fnq3::win::ScopedGlobalMemory hMem( GlobalAlloc( GMEM_MOVEABLE | GMEM_DDESHARE, length ) );
	if ( hMem ) {
		bool copied = false;
		{
			fnq3::win::ScopedGlobalLock<char> ptr( hMem.get() );
			if ( ptr ) {
...
void Sys_SetClipboardBitmap( const byte *bitmap, int length )
{
	...
	if ( hMem ) {
		bool copied = false;
		{
			fnq3::win::ScopedGlobalLock<byte> ptr( hMem.get() );
			if ( ptr ) {
