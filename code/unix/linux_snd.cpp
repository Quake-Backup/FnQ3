...
static fnq3::posix::ScopedLibrary t_lib;
...
static fnq3::posix::ScopedLibrary a_lib;
...
static qboolean setup_ALSA( smode_t mode )
{
	...
	if ( !t_lib )
	{
		t_lib.reset( fnq3::posix::LoadLibraryFallback( "libpthread.so.0", "libpthread.so" ) );
		if ( !t_lib )
		{
	...
	if ( !fnq3::posix::LoadSymbols( t_lib.get(), t_list, "pthread", fnq3::posix::SymbolLoadFailure::Support ) )
	{
		UnloadLibs();
		return qfalse;
	}
	...
	if ( !a_lib )
	{
		a_lib.reset( fnq3::posix::LoadLibraryFallback( "libasound.so.2", "libasound.so" ) );
		if ( !a_lib )
		{
	...
	if ( !fnq3::posix::LoadSymbols( a_lib.get(), a_list, "ALSA", fnq3::posix::SymbolLoadFailure::Support ) )
	{
		goto __fail;
	}
#endif
...
static qboolean snd_inited = qfalse;
static fnq3::posix::ScopedFd audio_fd;
static int map_size;
