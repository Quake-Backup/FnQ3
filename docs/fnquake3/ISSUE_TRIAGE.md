...
4. reads repository context from `README.md`, `BUILD.md`, `docs/fnquake3/TECHNICAL.md`, and issue-relevant docs such as `docs/GLX.md`, `docs/AUDIO.md`, `docs/SCREENSHOTS.md`, and `docs/CONSOLE.md`
...
1. repository variable `FNQ3_ISSUE_TRIAGE_MODEL`
2. repository variable `FNQ3_RELEASE_NOTES_MODEL`
3. built-in fallback `openai/gpt-4.1`

The fallback keeps the existing repository convention used by `scripts/manual_release.py`, but maintainers should set `FNQ3_ISSUE_TRIAGE_MODEL` to the latest approved GPT model available in the repository's GitHub Models environment.
...
- `FNQ3_ISSUE_TRIAGE_DRY_RUN`

`FNQ3_ISSUE_TRIAGE_DRY_RUN=true` keeps the workflow read-only apart from normal API fetches, which is useful while tuning prompts and thresholds.
...
python3 .github/scripts/issue_triage.py triage --repo themuffinator/FnQ3 --issue-number 3 --dry-run
```
