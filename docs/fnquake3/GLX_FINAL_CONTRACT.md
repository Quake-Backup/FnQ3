...
FnQuake3 preserves retail Quake III Arena compatibility, demo compatibility,
...
- Stable C ABI: the engine-facing boundary stays `REF_API_VERSION 8`,
...
- One front end, tiered executors: the front end compiles Quake/FnQ3 render
...
- Deterministic pass order: GLx preserves the id Tech 3 / FnQ3 render order
...
| Version | `REF_API_VERSION` remains `8` unless the whole engine ABI is deliberately versioned. |
...
| `MaterialIR` | id Tech 3 / FnQ3 shader intent with sort, blend, depth, alpha, fog, tcgen, texmod, and bundle semantics preserved. |
