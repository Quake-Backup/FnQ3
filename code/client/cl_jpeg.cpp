...
#	error Need libjpeg >= 80 or jpeg_mem_ support
#endif

/* Catching errors, as done in libjpeg's example.c */
struct q_jpeg_error_mgr_t {
...
static void CL_JPGErrorExit(j_common_ptr cinfo)
{
	...
	q_jpeg_error_mgr_t *jerr = reinterpret_cast<q_jpeg_error_mgr_t *>( cinfo->err );
  
	(*cinfo->err->format_message)( cinfo, buffer.data() );
  
	Com_Printf( "Error: %s", buffer.data() );
...
void CL_LoadJPG( const char *filename, unsigned char **pic, int *width, int *height )
{
	...
	*/
	...
	len = FS_ReadFile( const_cast<char *>( filename ), &fbuffer.v );
	if ( !fbuffer.b || len < 0 ) {
		return;
	}
...
#ifdef ZONE_DEBUG
  out = static_cast<byte *>( Z_MallocDebug( static_cast<int>( memcount ), const_cast<char *>( "jpeg pixels" ),
	  const_cast<char *>( __FILE__ ), __LINE__ ) );
#else
  out = static_cast<byte *>( Z_Malloc( memcount ) );
#endif
  ...
  *height = cinfo.output_height;
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
	...
	buffer = &buf;
    static_cast<void>( jpeg_read_scanlines( &cinfo, buffer, 1 ) );
  }
