...
static constexpr int MAX_DEFINEPARMS = 128;
...
static constexpr int DEFINEHASHSIZE = 1024;

[[maybe_unused]] static constexpr int TOKEN_HEAP_SIZE = 4096;
...
//		{ "__STDC__", BUILTIN_STDC },
	...
	{
		define = botlib::Alloc<define_t>();
		*define = {};
		define->name = botlib::CopyString(builtin[i].string);
...
static int PC_ExpandDefine(source_t *source, token_t *deftoken, define_t *define,
										token_t **firsttoken, token_t **lasttoken)
{
	token_t *parms[MAX_DEFINEPARMS] = { nullptr }, *dt, *pt, *t;
...
static int PC_Directive_define(source_t *source)
{
	...
	define = botlib::Alloc<define_t>();
	*define = {};
	define->name = botlib::CopyString(token.string);
	//add the define to the source
#if DEFINEHASHING
	PC_AddDefineToHash(define, source->definehash);
#else //DEFINEHASHING
...
#if DEFINEHASHING
	src.definehash = botlib::ClearedAlloc<define_t *>(DEFINEHASHSIZE);
#endif //DEFINEHASHING
...
// Changes Globals:		-
//============================================================================
static int PC_AddDefine(source_t *source, const char *string)
{
	define_t *define;
...
static define_t *PC_CopyDefine(source_t *source, const define_t *define)
{
	...
	newdefine = botlib::Alloc<define_t>();
	//copy the define name
	newdefine->name = botlib::CopyString(define->name);
...
static constexpr int MAX_VALUES = 64;
static constexpr int MAX_OPERATORS = 64;
...
// Changes Globals:		-
//============================================================================
static const char *PC_TokenTypeName(int type)
{
	switch (type)
	{
...
} //end of the function PC_TokenTypeName

static void PC_NumberSubtypeName(int subtype, char *str, size_t size)
{
	str[0] = '\0';
	if (subtype & TT_DECIMAL) Q_strncpyz(str, "decimal", size);
	if (subtype & TT_HEX) Q_strncpyz(str, "hex", size);
	if (subtype & TT_OCTAL) Q_strncpyz(str, "octal", size);
	if (subtype & TT_BINARY) Q_strncpyz(str, "binary", size);
	if (subtype & TT_LONG) Q_strcat(str, size, " long");
	if (subtype & TT_UNSIGNED) Q_strcat(str, size, " unsigned");
	if (subtype & TT_FLOAT) Q_strcat(str, size, " float");
	if (subtype & TT_INTEGER) Q_strcat(str, size, " integer");
} //end of the function PC_NumberSubtypeName
...
int PC_ExpectTokenType(source_t *source, int type, int subtype, token_t *token)
{
	...
	if (token->type != type)
	{
		SourceError(source, "expected a %s, found %s", PC_TokenTypeName(type), token->string);
		return qfalse;
	} //end if
	if (token->type == TT_NUMBER)
	{
		...
		{
			PC_NumberSubtypeName(subtype, str, sizeof(str));
...
#if DEFINEHASHING
	source->definehash = botlib::ClearedAlloc<define_t *>(DEFINEHASHSIZE);
#endif //DEFINEHASHING
...
static constexpr int MAX_SOURCEFILES = 64;
