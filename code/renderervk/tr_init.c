...
static void R_Register( void )
{
	...
	ri.Cvar_SetDescription( r_dlightShadowBias, "Maximum receiver bias in world units for angle-aware dynamic-light shadow-map sampling." );
	ri.Cvar_SetGroup( r_dlightShadowBias, CVG_RENDERER );
	r_dlightShadowCasterDepthBias = ri.Cvar_Get( "r_dlightShadowCasterDepthBias", "1.5", CVAR_ARCHIVE_ND );
	...
	r_dlightShadowCasterSlopeBias = ri.Cvar_Get( "r_dlightShadowCasterSlopeBias", "1.5", CVAR_ARCHIVE_ND );
	...
	r_dlightShadowCasterNormalBias = ri.Cvar_Get( "r_dlightShadowCasterNormalBias", "0.5", CVAR_ARCHIVE_ND );
	...
	r_csmShadows = ri.Cvar_Get( "r_csmShadows", "0", CVAR_ARCHIVE_ND );
	ri.Cvar_CheckRange( r_csmShadows, "0", "1", CV_INTEGER );
	ri.Cvar_SetDescription( r_csmShadows, "Enables directional cascaded shadow-map planning. Shadow resources and sampling remain separate follow-up work." );
	ri.Cvar_SetGroup( r_csmShadows, CVG_RENDERER );
	r_csmCascadeCount = ri.Cvar_Get( "r_csmCascadeCount", "4", CVAR_ARCHIVE_ND );
	ri.Cvar_CheckRange( r_csmCascadeCount, "1", XSTRING( CSM_MAX_CASCADES ), CV_INTEGER );
	ri.Cvar_SetDescription( r_csmCascadeCount, "Number of directional shadow cascades to plan." );
	...
	r_csmResolution = ri.Cvar_Get( "r_csmResolution", "1024", CVAR_ARCHIVE_ND );
	ri.Cvar_CheckRange( r_csmResolution, "128", "4096", CV_INTEGER );
	ri.Cvar_SetDescription( r_csmResolution, "Nominal per-cascade directional shadow-map resolution used for stable texel snapping." );
	...
	ri.Cvar_SetDescription( r_csmShadowBias, "Maximum receiver bias in world units for future directional shadow-map sampling." );
	...
	ri.Cvar_SetDescription( r_csmCasterDepthBias, "Constant depth bias units reserved for future directional shadow caster rendering." );
	...
	ri.Cvar_SetDescription( r_csmCasterSlopeBias, "Slope-scaled depth bias reserved for future directional shadow caster rendering." );
	...
	ri.Cvar_SetDescription( r_csmCasterNormalBias, "Maximum normal offset in world units reserved for future directional shadow caster rendering." );
	...
	ri.Cvar_SetDescription( r_csmDebug, "Prints directional cascaded shadow-map planning counters each frame." );
	...
	ri.Cvar_SetDescription( r_hudExcludePostProcess, "Exclude 3D HUD scenes (RDF_NOWORLDMODEL after the world view) from bloom and scene HDR post-processing where supported. Set to 0 for legacy post-processed HUD models." );
