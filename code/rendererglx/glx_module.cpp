	...
	char *out, size_t outSize )
{
	...
	const int written = std::snprintf( out, outSize,
		...
		"    if (u_DlightFactors.y > 0.5) {\n"
		"        vec3 spotDir = safeNormalize(u_LightVector.xyz);\n"
		"        vec3 lightToFrag = safeNormalize(-v_LightVector);\n"
		"        float spotCos = dot(lightToFrag, spotDir);\n"
		...
		"        spotFactor = (3.0 - 2.0 * spotFactor) * spotFactor * spotFactor;\n"
		"    } else {\n"
