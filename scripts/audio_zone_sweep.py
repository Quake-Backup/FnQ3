...
def default_tool_path() -> Path:
    env_tool = os.environ.get("FNQ3_AUDIOZONESC")
    if env_tool:
        return Path(env_tool)
    return Path("fnq3-audiozonesc")
...
def resolve_tool(tool: Path, dry_run: bool) -> str:
    ...
    raise FileNotFoundError(
        f"audio zone compiler '{name}' was not found; pass --tool or set FNQ3_AUDIOZONESC"
    )
...
def parse_args(argv: Sequence[str]) -> SweepOptions:
    parser = argparse.ArgumentParser(
        description="Batch-generate and audit FnQuake3 audio-zone sidecars from BSP maps.",
    )
