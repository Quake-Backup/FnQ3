...
GLx is FnQuake3's canonical OpenGL-lineage renderer. It is the renderer path where new OpenGL-family work lands: capability tiers, renderer switching proof, static-world acceleration, dynamic streaming, material execution, postprocess parity, output transforms, and GLx-specific diagnostics.
...
```sh
python scripts/glx_runtime_sweep.py --gate rc-parity --exe path/to/fnquake3 --basepath path/to/game
python scripts/glx_runtime_sweep.py --gate rc-proof --exe path/to/fnquake3 --basepath path/to/game --proof-dir .tmp/glx-proof/windows-x64
```
...
If a driver-specific issue blocks play, set `cl_renderer opengl` and run `vid_restart` to return to the legacy OpenGL renderer in current builds. In a promoted release, use the rollback package described in [GLX_PROMOTION.md](fnquake3/GLX_PROMOTION.md).
