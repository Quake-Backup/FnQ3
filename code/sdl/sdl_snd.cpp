...
static fnq3::sdl::ScopedAudioStream sdlPlaybackStream;
static fnq3::sdl::ScopedMutex sdlAudioMutex;

#if defined USE_VOIP
#define USE_SDL_AUDIO_CAPTURE

static fnq3::sdl::ScopedAudioStream sdlCaptureStream;
...
static void SNDDMA_AudioCallback( void *userdata, SDL_AudioStream *stream, int additional_amount, int total_amount )
{
	...
	while ( additional_amount > 0 )
	{
		...
		{
			fnq3::sdl::ScopedMutexLock lock( sdlAudioMutex.get() );
#ifdef USE_SDL_AUDIO_CAPTURE
...
qboolean SNDDMA_Init( void )
{
	...
	int sampleFrames;
	int tmp;
	fnq3::sdl::ScopedAudioStream playbackStream;
	fnq3::sdl::ScopedMutex audioMutex;
	fnq3::sdl::ScopedMemory<byte> dmaBuffer;
#ifdef USE_SDL_AUDIO_CAPTURE
	fnq3::sdl::ScopedAudioStream captureStream;
#endif
	...
	Com_Printf( "OK\n" );
	fnq3::sdl::ScopedSubSystem audioSubsystem( SDL_INIT_AUDIO, true );
