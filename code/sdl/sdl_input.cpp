...
static void IN_InitJoystick( void )
{
	...
	int total = 0;
	char buf[16384] = "";
	fnq3::sdl::ScopedSubSystem joystickSubsystem;
	fnq3::sdl::ScopedSubSystem gamepadSubsystem;
	...
	if ( !SDL_WasInit( SDL_INIT_JOYSTICK ) )
	{
		...
		joystickSubsystem = fnq3::sdl::ScopedSubSystem( SDL_INIT_JOYSTICK, true );
	...
	if ( !SDL_WasInit( SDL_INIT_GAMEPAD ) )
	{
		...
		gamepadSubsystem = fnq3::sdl::ScopedSubSystem( SDL_INIT_GAMEPAD, true );
		Com_DPrintf( "SDL_InitSubSystem(SDL_INIT_GAMEPAD) passed.\n" );
	}

	fnq3::sdl::ScopedSdlMemory<SDL_JoystickID> joysticks( SDL_GetJoysticks( &total ) );
	if ( !joysticks ) {
		total = 0;
	}
