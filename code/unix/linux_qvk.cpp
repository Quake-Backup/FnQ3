...
qboolean VK_CreateSurface( VkInstance instance, VkSurfaceKHR *surface )
{
	...
	qvkCreateXlibSurfaceKHR = reinterpret_cast<PFN_vkCreateXlibSurfaceKHR>( VK_GetInstanceProcAddr( instance, "vkCreateXlibSurfaceKHR" ) );
...
{
	GLW_ScopedLibrary lib( Sys_LoadLibrary( dllname ) );
	if ( lib )
	{
		qvkGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>( Sys_LoadFunction( lib.get(), "vkGetInstanceProcAddr" ) );
