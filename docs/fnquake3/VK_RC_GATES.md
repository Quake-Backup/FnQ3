...
| `vk-modern` | Modern Vulkan path for FBO/HDR/bloom/MSAA, ACES tone mapping, exposure-aware bloom extraction, sync2/dynamic-rendering reporting, GPU timings, and timedemo data. | `vk-modern` | `q3dm1`, `q3dm17`, `demo1` | Screenshots are written, `vkinfo` is clean, `r_speeds 7` produces GPU timing spans, and timedemo metrics are captured. |
| `vk-hdr` | Native-HDR request path for HDR10 swapchain negotiation on capable display paths, using the same ACES tone scale and bloom extraction profile as the modern SDR gate. | `vk-hdr` | `q3dm1` | The HDR request is visible in `vkinfo` as active or requested-unavailable, with screenshot output intact. |
...
- `vkinfo` reports pipeline-cache state, HDR state, tone-map and bloom state, sync2/dynamic-rendering state, barrier counts, descriptor counters, command-pool resets, and memory counters.
...
python scripts/vk_runtime_sweep.py --gate vk-smoke --exe path/to/fnquake3.exe --basepath path/to/game/root
python scripts/vk_runtime_sweep.py --gate vk-modern --exe path/to/fnquake3.exe --basepath path/to/game/root
python scripts/vk_runtime_sweep.py --gate vk-hdr --exe path/to/fnquake3.exe --basepath path/to/game/root
```
