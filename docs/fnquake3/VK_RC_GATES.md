...
- `vkinfo` reports pipeline-cache state, HDR state, output-backend request/selection/native display state, tone-map and bloom state, sync2/dynamic-rendering state, barrier counts, descriptor counters, command-pool resets, and memory counters.
