...
static fnq3::sdl::ScopedGLContext SDL_glContext;
#ifdef USE_VULKAN_API
...
{
	fnq3::sdl::ScopedProperties props( SDL_CreateProperties() );
...
void GLW_UpdateWindowState( void )
{
	...
	int numDisplays = 0;
	fnq3::sdl::ScopedSdlMemory<SDL_DisplayID> displays( SDL_GetDisplays( &numDisplays ) );
	...
	int *x, int *y, int w, int h )
{
	...
	fnq3::window::Insets decorations{};
...
static rserr_t GLW_SetMode( int mode, const char *modeFS, qboolean fullscreen, qboolean vulkan )
{
	...
	{
#ifdef USE_ICON
		fnq3::sdl::ScopedSurface icon( SDL_CreateSurfaceFrom(
...
void GLimp_QueryDisplayOutput( rendererDisplayOutput_t *output )
{
	...
	size_t iccSize = 0;
	fnq3::sdl::ScopedSdlMemory<void> iccProfile;
	float sdrWhite;
	float hdrHeadroom;
	qboolean windowHdr;
	qboolean displayHdr;
...
#else
	char *data = NULL;
	fnq3::sdl::ScopedSdlMemory<char> cliptext( SDL_GetClipboardText() );
...
void Sys_SetClipboardBitmap( const byte *bitmap, int length )
{
	...
	fnq3::win::ScopedClipboard clipboard( NULL );
	if ( !clipboard ) {
		return;
	}
	...
	fnq3::win::ScopedGlobalMemory memory( GlobalAlloc( GMEM_MOVEABLE | GMEM_DDESHARE, length ) );
	if ( !memory ) {
		return;
	}

	{
		fnq3::win::ScopedGlobalLock<byte> lock( memory.get() );
		if ( !lock ) {
			return;
		}
