...
- [`docs/fnquake3/FOG_RENDERING.md`](./FOG_RENDERING.md) records the shared analytic fog curve, optional global map-sidecar layer, GLx/Vulkan execution changes, fixed-function fallback, performance effects, and volumetric-fog compatibility boundary.
...
- Optional global-fog sidecars are tracked as text `pkg/baseq3/maps/*.fog` and `pkg/missionpack/maps/*.fog` files and packaged at their matching virtual `maps/` paths. They are visual-only input for the GLx/OpenGL-lineage and Vulkan framebuffer renderers; `q3tourney5` (Fatal Instinct) deliberately has no preset because it already carries native fog.
