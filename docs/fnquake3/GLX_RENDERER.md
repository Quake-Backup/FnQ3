...
- `r_glxStreamDrawShadows` for separately opting stencil shadow-volume passes into streamed draw submission.
- `r_glxStreamDrawBeams` for separately opting immediate beam entity draw-array passes into streamed draw submission.
- `r_glxStreamDrawPostProcess` for separately opting fullscreen postprocess draw-array passes into streamed draw submission.
...
- `glx_module.*`: the C ABI bridge used behind the renderer compatibility facade. The bridge header forward-declares the legacy ABI pointer types it needs instead of including `tr_public.h`; new legacy-renderer touch points should still be added through `tr_glx_compat.h` rather than including this module header directly. Shared ids and small public payload structs belong in `renderercommon/tr_glx_public.h` instead of this ABI header.
...
| `rc` | Conservative release-candidate surface: VBO world cache, GLx world renderer bundle, ordered packet-batch static spans, stream draw for proven stage shapes including CPU-computed texmods and environment coordinates, material renderer, final-pass bloom parity, and GPU timing. Indirect static-world stress paths and high-risk dynamic-light/screen-map/video-map/shadow/beam stream gates stay off. |
...
`fnq3_glx_logic_tests` provides the first renderer-focused pure logic test target. It runs without an OpenGL context, without the legacy `code/renderer` include path, and without retail assets. It covers the RC material permutation key classifier, special scene flags staying gate-only instead of expanding the shader permutation table, unsupported multitexture combine rejection, capability-tier/version/extension logic, stream strategy fallback selection, stream draw material-gate allowlists including explicit dynamic-light, screen-map, video-map, shadow-volume, and beam gating, static-world packet classification, static draw-policy gating, and the developer-only broad key-mode escape hatch. `fnq3_glx_header_boundary` complements it by scanning the pure GLx headers and the public module bridge for accidental dependencies on `qgl`, `tr_public.h`, renderer ABI implementation types, OpenGL object typedefs, module-local headers, or the legacy renderer include tree:
