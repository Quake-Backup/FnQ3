...
| Static map lights | `maps/<map>.lights.json` point/spot definitions are loaded, PVS/area filtered, budgeted, reloadable, and promoted into the existing light path. Their `castsShadows` policy is retained for RT visibility without adding a raster shadow atlas. |
